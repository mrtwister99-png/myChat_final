...
const formatLastSeen = (timestamp) => {
  ...
  if (diffMinutes < 1) return 'Aktivní před chvílí';
  if (diffMinutes < 60) return `Aktivní před ${diffMinutes} min`;

  const diffHours = Math.floor(diffMinutes / 60);
  if (diffHours < 24) return `Aktivní před ${diffHours} h`;

  return `Aktivní před ${Math.floor(diffHours / 24)} d`;
};

const AdminChat = ({ navigation, route }) => {
  // FIX: ID normalizujeme na string, server ho tak posila
  const userId = String(route?.params?.userId || 'unknown-user');
  ...
  useEffect(() => {
    getStoredValue('DESTRUCTIVE_MODE')
      .then((value) => setDestructiveMode(value === 'true'))
      .catch(() => {});
  }, []);
...
useEffect(() => {
  ...
  useEffect(() => {
    const handleConnect = () => {
      ...
      try {
        // Reautentizace používá pouze PIN načtený po platném přihlášení.
        // jinak po reconnectu socket ztratil roli admina a server tise
        // zahazoval chat:send, chat:react i zmeny profilu
        const adminPinToUse = String(globalThis.CUSIIK_ADMIN_PIN || '').replace(/[^0-9]/g,'').slice(0,5);
    ...
    const handleServerState = (serverState) => {
      ...
      if (typeof serverState?.destructiveMode === 'boolean') {
        setDestructiveMode(serverState.destructiveMode);
      }
    ...
    const handleChatMessages = ({ userId: incomingUserId, messages: nextMessages, readAt }) => {
      const cleanIncomingId = String(incomingUserId || '').trim();
      if (!cleanIncomingId) return;
      const chats = getGlobalChats();
      const safeMessages = (nextMessages || []).filter(
        (item) => !String(item?.text || '').startsWith(ANNOUNCEMENT_PREFIX)
      );
      chats[cleanIncomingId] = safeMessages;
      if (cleanIncomingId === String(userId)) {
        const userMessageCount = safeMessages.filter((item) => item?.sender === 'user').length;
        const nextReadCounts = { ...getGlobalAdminReadCounts(), [userId]: userMessageCount };
  ...
  const toggleDestructiveMode = async () => {
    const nextValue = !destructiveMode;
    setDestructiveMode(nextValue);
    await setStoredValue('DESTRUCTIVE_MODE', String(nextValue)).catch(() => {});
    socket.emit('admin:setDestructiveMode', { enabled: nextValue });
  };

  const sendFootMessage = () => {
    if (selectionMode || isMuted) {
      return;
    }
      ...
      >
        ...
        >
          ...
          <View style={styles.topPanel}>
            ...
            <View style={styles.userInfoBox}>
              ...
              <Text
                style={[
                  styles.presenceText,
                  currentUserData?.online ? styles.presenceOnline : styles.presenceOffline,
                ]}
              >
                {currentUserData?.online
                  ? '● Online'
          ...
          <View style={styles.statusBar}>
            ...
            <Pressable
              accessibilityLabel="Destruktivní režim"
              onPress={toggleDestructiveMode}
              style={[styles.destructiveSwitch, destructiveMode && styles.destructiveSwitchActive]}
            >
              <View style={[styles.destructiveKnob, destructiveMode && styles.destructiveKnobActive]} />
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  // FIX: stejny krizek jako v liste nahore
  modalCloseButton: {
    width: 22,
    height: 22,
    marginLeft: 4,
  ...
  // FIX: tenhle styl chybel uplne -> obrazek se vykreslil v plne velikosti (obri krizek)
