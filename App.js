...
const App = () => {
  useEffect(() => {
    const init = async () => {
      ...
      // Načteme poslední platné PINy pro opětovné ověření po reconnectu.
      // zustal bez role -> zpravy, reakce ani zmeny profilu uz neprosly.
      const [lastUserId, lastUserName, savedUserPin, savedAdminPin] = await Promise.all([
        AsyncStorage.getItem('lastUserId'),
        AsyncStorage.getItem('lastUserName'),
        AsyncStorage.getItem('userPin'),
        AsyncStorage.getItem('adminPin'),
      ]);

      if (savedUserPin) {
        globalThis.CUSIIK_USER_PIN = savedUserPin;
      }

      if (savedAdminPin) {
        globalThis.CUSIIK_ADMIN_PIN = savedAdminPin;
      }
      const deviceId = await getOrCreateDeviceId();
      globalThis.CUSIIK_DEVICE_ID = deviceId;
      ...
      if (socket.connected) {
        socket.emit('client:ready', { lastUserId: lastUserId || null, deviceId });
      }
  ...
  useEffect(() => {
    const sub = addNotificationResponseListener((data, response) => {
      ...
      const chatScreen = role === 'admin' ? 'AdminChat' : 'UzivatelPin';

      if (actionId === 'reply') {
        if (replyText && notificationUserId) {
          const sendReply = () => {
          ...
          // appka mohla byt na pozadi -> socket muze byt odpojeny
          if (socket.connected) {
            sendReply();
          } else {
            socket.once('connect', () => setTimeout(sendReply, 400));
            socket.connect();
          }
        }

        // odpoved se odeslala, notifikaci schovat
        dismissNotification(response?.notification?.request?.identifier);
        return;
      }

      if (notificationUserId && navigationRef.isReady()) {
        // openChat = casove razitko, aby UzivatelPin prepnul do chatu
        // i kdyz uz na te obrazovce je (stejne params by nic neudelaly)
        navigationRef.navigate(chatScreen, { userId: notificationUserId, openChat: Date.now() });
      }
    });
    return () => sub?.remove?.();
  }, []);
